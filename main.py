...
# New response models for separate endpoints
...
class ScoreResponse(BaseModel):
    results: List[ScoreResult] = Field(..., description="Score results for each user")
...
class SkillsResponse(BaseModel):
    results: List[SkillsResult] = Field(..., description="Skills analysis results for each user")
...
# Extract only score information from results
def extract_score_results(full_results):
    score_results = []
...
# Extract only skills information from results
def extract_skills_results(full_results):
    skills_results = []
    for result in full_results:
        skills_results.append({
            "user_id": result["user_id"],
            "missing_keywords": result["missing_keywords"],
            "sgAnalysis": result["sgAnalysis"]
        })
...
# New endpoint: Analyze only scores
@app.post("/analyze-score", response_model=ScoreResponse,
          summary="Get only score analysis for resumes",
          description="Provide resume image URLs with user IDs and a job description to get score analysis only")
async def analyze_scores_endpoint(
        request: ATSRequest,
        api_key: str = Depends(get_api_key)
):
    if not request.resumes:
    ...
    logger.info(f"Received score analysis request with {len(request.resumes)} resumes")
    full_results = await process_resumes(request.resumes, request.job_description, api_key)
    score_results = extract_score_results(full_results)
    logger.info(f"Score analysis complete for {len(score_results)} resumes")
    return {"results": score_results}


# New endpoint: Analyze only skills and gap analysis
@app.post("/analyze-skills", response_model=SkillsResponse,
          summary="Get only skills analysis for resumes",
          description="Provide resume image URLs with user IDs and a job description to get skills and gap analysis only")
async def analyze_skills_endpoint(
    ...
    logger.info(f"Received skills analysis request with {len(request.resumes)} resumes")
    full_results = await process_resumes(request.resumes, request.job_description, api_key)
    skills_results = extract_skills_results(full_results)
    logger.info(f"Skills analysis complete for {len(skills_results)} resumes")
    return {"results": skills_results}
...
# New form-based endpoint for scores only
@app.post("/analyze-score-form", response_model=ScoreResponse,
          summary="Get only score analysis for resumes (form-based)",
          description="Provide resume URLs with user IDs and a job description using form data to get score analysis only")
async def analyze_scores_form(
        job_description: str = Form(...),
        resume_urls: str = Form(...),  # Comma-separated URLs
        user_ids: str = Form(...),  # Comma-separated user IDs
        api_key: str = Depends(get_api_key)
    ...
    logger.info(f"Received form-based score analysis request with {len(urls)} resumes")

    # Convert to ResumeInput objects with user IDs
    resume_inputs = [ResumeInput(url=url, user_id=uid) for url, uid in zip(urls, ids)]

    full_results = await process_resumes(resume_inputs, job_description, api_key)
    score_results = extract_score_results(full_results)
    logger.info(f"Form-based score analysis complete for {len(score_results)} resumes")
    return {"results": score_results}


# New form-based endpoint for skills analysis only
@app.post("/analyze-skills-form", response_model=SkillsResponse,
          summary="Get only skills analysis for resumes (form-based)",
          description="Provide resume URLs with user IDs and a job description using form data to get skills analysis only")
async def analyze_skills_form(
        job_description: str = Form(...),
        resume_urls: str = Form(...),  # Comma-separated URLs
        user_ids: str = Form(...),  # Comma-separated user IDs
        api_key: str = Depends(get_api_key)
    ...
    logger.info(f"Received form-based skills analysis request with {len(urls)} resumes")
    ...
    full_results = await process_resumes(resume_inputs, job_description, api_key)
    skills_results = extract_skills_results(full_results)
    logger.info(f"Form-based skills analysis complete for {len(skills_results)} resumes")
    return {"results": skills_results}
